...
class Epub():
    ...
    @staticmethod
    def from_html(logger, dir_in, dir_out):
        ...
        Filesystem.copytree(logger, dir_in, epub_dir)
        ...
        if not contentref:
            logger.info("Could not find content file...")
            return None

        xslt = Epub.html_to_opf(
            logger, temp_opf, os.path.join(epub_dir, "package.opf"))
        if not xslt.success:
            return None

        # create dir_out/EPUB/nav.xhtml based on input html (xslt)
        xslt = Epub.html_to_nav(logger, html_file,
